...
//		useMaximumFinder = gd.getNextBoolean();
	...
	public void loadSegmentationByImage(ImagePlus segmentation, ImagePlus intensityImp) {
		...
		for (int i=0; i<stack.getSize(); ++i) {
			...
			comps.filterComponents(minArea, maxArea, minCircularity, maxCircularity, 0, 1000, false);
			tracking.addComponentsAnalysis(comps);
		}
	}
	
	public void loadSegmentationByPath(Path segmentationResultPath, ImagePlus intensityImp) {
		ImagePlus segmentation = new ImagePlus(segmentationResultPath.toString());
		loadSegmentationByImage(segmentation, intensityImp);
	}
