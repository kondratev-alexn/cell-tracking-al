...
public class Hessian {
	...
	public void calculateHessian(float sigma) {
		...
		Iyy = ip.duplicate();
		/*
		 * sigma = (float) Math.sqrt(sigma); gaus.GaussianDerivativeX(Ixx, sigma); Ixy =
		 * Ixx.duplicate(); gaus.GaussianDerivativeX(Ixx, sigma);
		 * gaus.GaussianDerivativeY(Ixy, sigma); gaus.GaussianDerivativeY(Iyy, sigma);
		...
		for (int i = 0; i < ip.getPixelCount(); i++) {
			...
			lambda1.setf(i, sigma * sigma * (float) ((Pxx + Pyy + det) / 2));
			lambda2.setf(i, sigma * sigma * (float) ((Pxx + Pyy - det) / 2));
		}
	}
	...
	/*
	 * test function to find better lambdas function. Some comments: lambda2 is
	 * negative, with large absolute value along the edges, zero on the edges.
	 * lambda 1 has large values in the center of the blobs if sigma is right. So mb
	 * l
	 */
	...
	/*
	 * get correct lambdas ration; if one f the values is less than threshold, then
	 * the result is zero
	 */
