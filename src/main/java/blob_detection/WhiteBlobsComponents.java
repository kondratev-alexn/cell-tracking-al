...
public class WhiteBlobsComponents {
	...
	final private int maxDistance = 150; // for blob's sortValue
	...
	public WhiteBlobsComponents(ImageProcessor intensityImage, int blobsCount) {
		...
		blobs = detector.findBlobsByLocalMaximaAsPoints(0.001f, true, blobsCount, 2, 2, false);
	...
	public void distributeBlobsBetweenDetections(ArrayList<WhiteBlobsDetection> detections) {
		/*
		 * algorithm: for each detection, sort blobs by distance-value, remember best N
		 * blobs add blobs that are best for only 1 detection to the corresponding
		 * detection for each blob that is best for several components: allot it to the
		 * detection with best distance-value for it between candidate detections DONE!
	...
	private void distributeBlobs(ArrayList<WhiteBlobsDetection> detections,
			ArrayList<ArrayList<Integer>> allBlobIndexesDetection) {
	...
	private boolean isIndexUnique(int index, ArrayList<ArrayList<Integer>> allBlobIndexesDetection,
			int detectionIndex) {
		ArrayList<Integer> indexes;
	...
	private boolean addBlobToBestDetection(int blobIndex, ArrayList<WhiteBlobsDetection> detections,
			ArrayList<ArrayList<Integer>> allBlobIndexesDetection) {
		ArrayList<Integer> indexes;
